package com.noinnion.android.newsplus.extension.theoldreader;
...
import com.noinnion.android.newsplus.extension.theoldreader.util.Utils;
...
public class TheOldReaderClient extends ReaderExtension {

	public static final String	TAG								= "TheOldReaderClient";

	public static final String	URL_LOGIN						= "https://theoldreader.com/accounts/ClientLogin";
	public static final String	URL_BASE_						= "https://theoldreader.com/reader";
	...
	public TheOldReaderClient() {}

	public TheOldReaderClient(Context c) {
		mContext = c;
	}
...
//	}
	...
	private Reader readStreamContents(long syncTime, long startTime, IItemListHandler handler, String continuation) throws IOException, ReaderException, RemoteException {
		...
		buff.append("?output=json&client=newsplus&ck=").append(syncTime);
		...
		// start time
		if (startTime > 0) buff.append("&ot=").append(startTime);
		...
		if (limit > 0) {
			buff.append("&n=").append(limit > SYNC_MAX_ITEMS ? SYNC_MAX_ITEMS : limit); // google only allows max 1000 at once
		}
	...
	private Reader readStreamItemIds(long syncTime, IItemIdListHandler handler) throws IOException, ReaderException, RemoteException {
		...
		// limit
		int limit = handler.limit();
		if (limit > 0) buff.append("&n=").append(limit);
		...
		while (jp.nextToken() != JsonToken.END_OBJECT) {
			...
			if (currName.equals("itemRefs")) { // contains an object
				...
				while (jp.nextToken() != JsonToken.END_ARRAY) {
					...
					jp.nextToken();
					if (currName.equals("id")) {
						idList.add(jp.getText()); // convert dec to hex
					} else {
						jp.skipChildren();
					}
				}
